...
@app.after_request
def _cors(resp):
    """Allow the deployed frontend (a different origin on Netlify) to call this API
    directly. Harmless no-op when frontend and API share an origin, e.g. via the
    Netlify `_redirects` proxy — see netlify.toml."""
...
if __name__ == '__main__':
    # 0.0.0.0 so this also binds correctly on a hosted platform (Render/Railway/Fly),
    # which routes traffic to the container on the port it assigns via $PORT.
    app.run(host='0.0.0.0', port=int(os.environ.get('PORT', 8100)), debug=False, threaded=True)
