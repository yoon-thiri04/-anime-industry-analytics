...
async function api(path, opts) {
  ...
  try { res = await fetch(path, opts); }
    ...
    /* the browser never got a response at all — on a static Netlify deploy this
       means netlify.toml's /api/* redirect still points at the placeholder, or
       the API host is down/asleep. A non-2xx JSON error below is a different,
       more specific case and keeps its own message. */
    throw new Error('Live API unreachable — set the backend URL in netlify.toml (see README).');
  }
